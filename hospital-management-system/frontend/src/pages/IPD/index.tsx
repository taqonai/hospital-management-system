  ...
  useEffect(() => {
    ...
    const fetchAdmission = async () => {
      try {
        ...
        console.log('Admission data loaded:', response.data.data);
        setAdmission(response.data.data);
      } catch (error: any) {
        console.error('Failed to fetch admission:', error);
        const errorMessage = error?.response?.data?.message || error?.message || 'Failed to load patient details';
            ...
            <div className="p-6 space-y-6">
              ...
              {activeTab === 'overview' && (() => {
                ...
                <div className="space-y-6">
                  ...
                  console.error('Error rendering overview tab:', renderError);
                  return (
                    <div className="p-6 bg-red-50 border border-red-200 rounded-xl">
                      <div className="flex items-start gap-3">
                        <ExclamationTriangleIcon className="h-6 w-6 text-red-600 flex-shrink-0 mt-1" />
                        <div>
                          <p className="font-medium text-red-900 mb-1">Error displaying patient details</p>
                          <p className="text-sm text-red-700 mb-3">{renderError.message || 'Unknown error'}</p>
                          <button
                            onClick={() => setActiveTab('orders')}
                            className="text-sm px-3 py-1 bg-white border border-red-300 rounded hover:bg-red-50"
                          >
                            Try another tab
                          </button>
                        </div>
