...
const updateProfileSchema = z.object({
  body: z.object({
    ...
    phone: z.string().min(8, 'Phone number must be at least 8 digits').max(15, 'Phone number must not exceed 15 digits').optional().nullable(),
    email: z.string().email('Please enter a valid email address').optional().or(z.literal('')).nullable(),
    address: z.string().max(200, 'Address is too long (max 200 characters)').optional().nullable(),
    city: z.string().max(50, 'City name is too long (max 50 characters)').optional().nullable(),
    state: z.string().max(50, 'State name is too long (max 50 characters)').optional().nullable(),
    zipCode: z.string().max(10, 'ZIP code is too long (max 10 characters)').optional().nullable(),
    emergencyContact: z.string().max(100, 'Emergency contact name is too long (max 100 characters)').optional().nullable(),
    emergencyPhone: z.string().max(15, 'Emergency phone must not exceed 15 digits').optional().or(z.literal('')).nullable(),
    occupation: z.string().max(100, 'Occupation is too long (max 100 characters)').optional().nullable(),
    nationality: z.string().max(50, 'Nationality is too long (max 50 characters)').optional().nullable(),
